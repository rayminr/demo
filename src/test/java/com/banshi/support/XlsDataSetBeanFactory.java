...
public class XlsDataSetBeanFactory {
    ...
    public static <T> List<T> createBeans(String file, String tableName, Class<T> clazz, Class<?> testClass) throws Exception {
        ...
        for (Map<String, Object> props : propsList) {
            ...
            beanUtils.populateExt(bean, props);
            beans.add(bean);
        }
        return beans;
    }

    // 从DbUnit的EXCEL数据集文件中创建单个bean
    public static <T> T createBean(String file, String tableName, Class<T> clazz, Class<?> testClass) throws Exception {
        BeanUtils beanUtils = createBeanUtils();
        List<Map<String, Object>> propsList = createProps(file, tableName, testClass);
        T bean = clazz.newInstance();
        beanUtils.populateExt(bean, propsList.get(0));
        return bean;
    }

    private static List<Map<String, Object>> createProps(String file, String tableName, Class<?> testClass) throws IOException,
            DataSetException {
        ...
        for (int i = 0; i < table.getRowCount(); i++) {
            ...
            for (Column c : columns) {
                ...
                String propName = underlineToCamel(c.getColumnName().toLowerCase());
                //String propName = c.getColumnName();
    ...
    private static BeanUtils createBeanUtils() {
        ConvertUtilsBean convertUtilsBean = createConverUtils();
        return new BeanUtils(convertUtilsBean);
    }
